...
interface Reel {
  ...
  user: string;
  caption: string;
  likes: number; // Changé en number pour pouvoir incrémenter
...
export default function FeedPage() {
  ...
  const [posts, setPosts] = useState<Post[]>([
    ...
    { id: 2, user: "Sarah_Code", content: "L'interface est super fluide avec le typage TS.", likes: 15 }
  ])

  const [reels, setReels] = useState<Reel[]>([
    { id: 1, user: "DesignMaster", caption: "Mon nouveau setup 2026 💻", likes: 1200, videoUrl: null },
    { id: 2, user: "ReactGuru", caption: "Next.js 15 est incroyable ! 🔥", likes: 850, videoUrl: null }
  ])
  ...
  const handleComment = (id: number) => {
    ...
    if (text) alert(`Commentaire posté sur #${id} : ${text}`)
  }
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      {activeTab !== 'play' && (
        <nav className="p-5 flex justify-between items-center sticky top-0 bg-[#0b0e14]/90 backdrop-blur-xl z-[100] border-b border-white/5">
          ...
          <div className="flex gap-5 items-center">
             <div className="bg-white/5 px-4 py-1.5 rounded-full text-[10px] font-black uppercase text-gray-400 tracking-widest">Shoncs</div>
        ...
        {activeTab === 'add' && (
          <div className="animate-in zoom-in-95 duration-300">
            <div className="bg-[#161b22] border border-white/10 rounded-[40px] p-8 relative shadow-2xl">
               ...
               <textarea value={postText} onChange={(e) => setPostText(e.target.value)} placeholder="Partage ton code ou une vidéo..." className="w-full bg-transparent min-h-[120px] outline-none text-lg resize-none" />
               <div className={`mt-4 border-2 border-dashed rounded-2xl p-6 transition-all flex flex-col items-center gap-2 relative ${videoFile ? 'border-blue-500 bg-blue-500/5' : 'border-white/10 hover:border-white/20'}`}>
                  {videoFile ? (
                    <div className="text-center font-black text-[10px] uppercase text-blue-500">Vidéo sélectionnée : {videoFile.name}</div>
                  ) : (
                    <>
                      <Icons.Video />
                      <span className="text-[9px] font-black uppercase text-gray-500 tracking-widest">Ajouter un Reel</span>
                      <input type="file" accept="video/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => { if(e.target.files && e.target.files[0]) setVideoFile(e.target.files[0]) }} />
                    </>
                  )}
               </div>
               <button onClick={handlePublish} className="w-full mt-6 bg-blue-600 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-blue-600/30 active:scale-95 transition-all">Publier</button>
            </div>
          </div>
        )}
            ...
            {reels.map(reel => (
              <div key={reel.id} className="h-screen w-full snap-start relative flex flex-col justify-end pb-36 p-6">
                ...
                <div className="relative z-10 space-y-3 max-w-[80%]">
                  ...
                  <p className="text-sm font-medium drop-shadow-md">{reel.caption}</p>
                </div>
                <div className="absolute right-4 bottom-40 z-10 flex flex-col gap-6">
                  <button onClick={() => handleLikeReel(reel.id)} className="flex flex-col items-center gap-1 group">
                    <div className="p-3 bg-white/10 rounded-full group-hover:text-red-500 transition-colors"><Icons.Heart /></div>
                    <span className="text-[10px] font-black">{reel.likes}</span>
                  </button>
                  <button onClick={() => handleComment(reel.id)} className="flex flex-col items-center gap-1">
                    <div className="p-3 bg-white/10 rounded-full"><Icons.Message /></div>
            ...
            {filteredPosts.map(post => (
              <div key={post.id} className="bg-[#161b22] border border-white/10 rounded-[35px] p-6 hover:border-white/20 transition-all">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-700 shadow-lg" />
                ...
                <div className="mt-5 pt-4 border-t border-white/5 flex gap-6 text-[10px] font-black uppercase tracking-widest text-white/30">
                  <button onClick={() => handleLikePost(post.id)} className="hover:text-red-500 transition-colors flex items-center gap-1.5">
                    ❤️ {post.likes}
                  </button>
                  <button onClick={() => handleComment(post.id)} className="hover:text-blue-500 transition-colors flex items-center gap-1.5">
                    💬 Commenter
                  </button>
                </div>
              </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-sm bg-[#161b22]/90 backdrop-blur-2xl border border-white/10 rounded-full p-2 flex justify-around items-center shadow-2xl z-[100]">
        <button onClick={() => setActiveTab('home')} className={`p-4 transition-all ${activeTab === 'home' ? 'text-blue-500 scale-110' : 'text-white/30'}`}><Icons.Home /></button>
        <button onClick={() => setActiveTab('search')} className={`p-4 transition-all ${activeTab === 'search' ? 'text-blue-500 scale-110' : 'text-white/30'}`}><Icons.Search /></button>
        <button onClick={() => setActiveTab('add')} className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${activeTab === 'add' ? 'bg-red-500 rotate-45' : 'bg-blue-600 hover:shadow-blue-600/50 shadow-lg'}`}>
          <span className="text-3xl font-light mb-1">+</span>
        </button>
        <button onClick={() => setActiveTab('play')} className={`p-4 transition-all ${activeTab === 'play' ? 'text-blue-500 scale-110' : 'text-white/30'}`}><Icons.Play /></button>
