...
export default function FeedPage() {
  ...
  // --- ACTIONS ---
  const backToHome = () => { setActiveTab('home'); stopCamera(); setPostText(""); setMediaFile(null); }
    ...
    return (
      <div className="min-h-screen bg-[#0b0e14] flex items-center justify-center p-6 text-white font-sans">
        <div className="w-full max-w-sm bg-[#161b22] rounded-[40px] p-10 border border-white/5 shadow-2xl animate-in zoom-in-95 text-center">
          ...
          <p className="text-xs opacity-40 mb-10 font-black uppercase tracking-widest">Créer ton Identifiant</p>
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-white font-sans">
      <main className="max-w-xl mx-auto p-4 pb-40">
        ...
        {activeTab === 'search' && (
          <div className="space-y-6 animate-in fade-in">
             <div className="flex items-center gap-4">
               ...
               <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Rechercher..." className="flex-1 bg-[#161b22] p-5 rounded-3xl outline-none border border-white/10" />
             </div>
        ...
        {activeTab === 'home' && (
          <div className="space-y-10 animate-in fade-in">
            {myPosts.length === 0 && <div className="text-center py-20 opacity-20 font-black uppercase tracking-widest">Rien à afficher</div>}
            {myPosts.map(post => (
              <div key={post.id} className="bg-[#161b22] rounded-[40px] border border-white/5 overflow-hidden shadow-xl">
                ...
                <div className="p-7">
                  ...
                  <div className="flex gap-8">
                    <button onClick={() => toggleLike(post.id)} className="flex items-center gap-1"><Icons.Heart active={post.isLiked} /><span className="text-xs opacity-30">{post.likes}</span></button>
                    <button onClick={() => setShowCommentsFor(showCommentsFor === post.id ? null : post.id)} className="flex items-center gap-1"><Icons.Message /><span className="text-xs opacity-30">{post.comments?.length || 0}</span></button>
                  </div>
                  {showCommentsFor === post.id && (
                    <div className="mt-6 pt-6 border-t border-white/5 space-y-4">
                      ...
                      <div className="flex gap-2">
                        <input value={commentInput} onChange={(e) => setCommentInput(e.target.value)} placeholder="Écrire..." className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2 text-xs outline-none" />
        ...
        {activeTab === 'add' && (
          <div className="bg-[#161b22] rounded-[45px] p-8 border border-white/5">
             <button onClick={backToHome} className="mb-6 flex items-center gap-2 text-[10px] font-black uppercase opacity-40"><Icons.ArrowLeft /> Retour</button>
            {!isCameraActive ? (
              <div className="space-y-6">
                <textarea value={postText} onChange={(e) => setPostText(e.target.value)} placeholder="Partager..." className="w-full bg-transparent outline-none text-2xl font-bold min-h-[120px]" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button onClick={handlePublish} className="py-8 rounded-3xl font-black uppercase shadow-xl" style={{ backgroundColor: themeColor }}>PUBLIER</button>
                </div>
              </div>
            ) : (
              <div className="relative aspect-[3/4] bg-black rounded-3xl overflow-hidden">
                ...
                <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-6 items-center">
                   ...
                   <button onClick={takePhoto} className="w-16 h-16 rounded-full border-4 border-white flex items-center justify-center transition-transform active:scale-90"><div className="w-12 h-12 bg-white rounded-full"></div></button>
        ...
        {activeTab === 'profile' && (
          <div className="animate-in fade-in">
            ...
            <div className="flex flex-col items-center mb-10">
              ...
              <h2 className="text-2xl font-black italic uppercase">@{username}</h2>
            </div>
            <div className="flex justify-center gap-12 mb-10 text-center">
               <div><p className="text-2xl font-black">{totalViews}</p><p className="text-[10px] uppercase opacity-30 font-bold">Vues</p></div>
               <div><p className="text-2xl font-black">{totalLikes}</p><p className="text-[10px] uppercase opacity-30 font-bold">Likes</p></div>
               <div onClick={handleFollowToggle} className="cursor-pointer active:scale-95 transition-all">
                 <p className="text-2xl font-black" style={{ color: themeColor }}>{followers}</p>
                 <p className="text-[10px] uppercase opacity-30 font-bold">Abonnés</p>
               </div>
            </div>
        ...
        {activeTab === 'settings' && (
          <div className="bg-[#161b22] rounded-[40px] p-8 border border-white/5 space-y-6 animate-in zoom-in-95">
             <div className="flex justify-between items-center mb-4">
               ...
               <button onClick={backToHome} className="p-2 bg-white/5 rounded-full"><Icons.ArrowLeft /></button>
             </div>
            <div><p className="text-[10px] font-black uppercase opacity-40 mb-2 ml-2">Identifiant</p><input value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold" /></div>
            <div><p className="text-[10px] font-black uppercase opacity-40 mb-2 ml-2">Couleur</p><input type="color" value={themeColor} onChange={(e) => setThemeColor(e.target.value)} className="w-full h-14 bg-transparent cursor-pointer" /></div>
            <button onClick={backToHome} className="w-full py-5 bg-white text-black rounded-2xl font-black uppercase text-xs shadow-lg">Sauvegarder & Retour</button>
          </div>
        )}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-[#161b22]/90 backdrop-blur-3xl border border-white/10 rounded-[40px] p-3 flex justify-around items-center z-[100] shadow-2xl">
        <button onClick={backToHome} className="p-4" style={{ color: activeTab === 'home' ? themeColor : 'white', opacity: activeTab === 'home' ? 1 : 0.3 }}><Icons.Home /></button>
        ...
        <button onClick={() => { setActiveTab('add'); if(!isCameraActive) startCamera(); }} className="w-16 h-16 rounded-full flex items-center justify-center text-3xl text-white shadow-xl active:scale-90 transition-transform" style={{ backgroundColor: themeColor }}>+</button>
